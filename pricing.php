...
<body>
				...
				<div class="container">
					...
					<div class="navbar-collapse collapse ">
						<ul class="nav navbar-nav">
							...
							<li class="active"><a href="pricing.php">Каталог</a></li>
						...
						<div class="nav-and-card">
							...
							<div class="cart-container">
								<a href="cart.php"><img class="garbage" src="img/garbage.png" alt=""></a>
	...
	<section id="content">
		...
		<div class="sneakers">
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<img alt='{$row['name']}' src='{$row['image']}' class='sneaker-image' />";
                    echo "<div class='sneaker-info'>";
                    echo "<h3>{$row['name']}</h3>";
                    echo "<p>{$row['description']}</p>";
                    echo "<p>Цена: {$row['price']} руб.</p>";
                    echo "<button class='btn-buy' onclick='addToCart(" . $row['id'] . ", \"" . $row['name'] . "\", " . $row['price'] . ")'>В корзину</button>";
                    echo "</div>";
                    echo "</div>";
                }
            } else {
		...
		<div class="sorting-block">
			<div class="sorting-option">
				<label for="size">РАЗМЕР ОБУВИ:</label>
				<div class="tags-container">
					<input type="checkbox" id="size36" name="size" value="36">
					<label for="size36">36</label>
					<input type="checkbox" id="size37" name="size" value="37">
					<label for="size37">37</label>
					<input type="checkbox" id="size38" name="size" value="38">
					...
					<input type="checkbox" id="size47" name="size" value="47">
					<label for="size47">47</label>


				</div>
				<a href="pricing.php" class="reload">Сбросить</a>
			</div>
			<div class="sorting-option">
			...
			<div class="sorting-option">
				...
				<div class="custom-select">
					<select id="category" name="category">
						<option value="" disabled selected>Выбрать</option>
						<option value="category1">Бег</option>
						<option value="category2">Спортивный стиль</option>
						<option value="category3">Баскетбол</option>
						<!-- Другие варианты категорий -->
					</select>
				</div>

				<div class="sorting-option">
					<label for="model">СЕЗОН</label>
					<div class="custom-select">
						<select id="model" name="model">
							<option value="" disabled selected>Выбрать</option>
							<option value="model1">Зима</option>
							<option value="model2">Весна</option>
							<option value="model3">Лето</option>
							<option value="model3">Осень</option>
	...
	<footer>
		<div class="container">
			...
			<!-- Модальное окно для отображения подробной информации о кроссовке -->
			<div id="sneakerModal" class="modal">
				<div class="modal-content">
					<span class="close">&times;</span>
					<div id="sneakerDetails">
						<!-- Сюда будет подставляться информация о кроссовке -->
					</div>
				</div>
			</div>
